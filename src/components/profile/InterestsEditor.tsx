...
export default function InterestsEditor({ items, onChange, isPending }: InterestsEditorProps) {
  ...
  function addInterest() {
    ...
    if (items.map((s) => s.toLowerCase()).includes(trimmed.toLowerCase())) {
      setError("That interest is already listed.");
      clearErrorAfter3s();
      return;
    }
  ...
  function removeInterest(interest: string) {
    onChange(items.filter((s) => s !== interest));
  }
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        {adding ? (
          <input
            ...
            className="px-3 py-1 rounded-full text-xs border border-[#3B1892] outline-none w-36 bg-white"
