...
function getBtnState(
  ...
  if (selectedIds.size === availableSections.length) return "full";
  return "partial";
}
...
export default function EnrollCard({ course }: { course: Course }) {
  ...
  const helperText = (() => {
    ...
    if (btnState === "full") return "You selected all sections";
    if (btnState === "partial")
      ...
      <div className="p-5">
        ...
        {!course.isEnrolled && (
          <button
            ...
            className="
    w-full mt-2.5 py-2.5 rounded-xl
    text-[12.5px] font-semibold text-violet-600
